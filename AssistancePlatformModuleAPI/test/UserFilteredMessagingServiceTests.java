...
public class UserFilteredMessagingServiceTests {
  ...
  public void test() throws Exception {
    ...
    MessagingService userFiltered =
        new UserFilteredMessagingServiceDecorator(new DummyMessagingService(), activationList);

    Channel<Position> c = userFiltered.channel("test", Position.class);

    List<Position> receivedData = new ArrayList<>();

    c.subscribeConsumer((channel, data) -> {
    ...
    for (int i = 0; i < events; i++) {
      ...
      c.publish(new Position(userId, userId, Calendar.getInstance().getTime(), Math.random(),
          Math.random()));
    }
