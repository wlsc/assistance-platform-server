...
public class BundleBootstrapper {
   ...
   */
  public static void bootstrap(ModuleBundle bundle, int restPort, String platformUrlAndPort,
      boolean localMode) {
    ...
    PlatformClient client = PlatformClientFactory.createInstance(platformUrlAndPort);
    ...
    actionRunner = new ModuleBundleClientActionRunnerProxy(bundle, actionRunner);
  ...
  private static void registerBundle(PlatformClient platformClient, ModuleBundle bundle) {
    ModuleBundleRegistrator registrator = new ModuleBundleRegistrator(bundle, platformClient);
  ...
  private static ISparkService createSparkService(ModuleBundle bundle, boolean localMode) {
    ...
    String defaultJarPath = System.getProperty("user.home") + "/" + bundle.getModuleId() + ".jar";
  ...
  private static String sparkMasterURL(ModuleBundle bundle) {
    return PlatformClientFactory.getInstance().getServiceConfig(bundle.getModuleId(),
        "spark").address[0];
  }
  ...
  public static void bootstrap(ModuleBundle bundle) {
    bootstrap(bundle, ModuleRestServer.DEFAULT_PORT,
        PlatformClientFactory.defaultPlatformUrlAndPort, false);
  }
}
