...
/**
 * Assistance Modules are the ones that interact with the user, give recommendations etc.. This
 * class should be sub-classed and then implement the desired Assistance Logic.
 * 
 * @author bjeutter
 *
 */
public abstract class AssistanceModule extends Module {
  ...
  protected final void internalDoBeforeStartup() {
    try {
      ...
      Logger.getLogger(AssistanceModule.class)
          .error("An error occured while starting the module rest server", e);
    }
  }
  ...
  /**
   * Gets the action runner which is required to perform actions on the client devices, like sending
   * messages etc.
   * 
   * @return
   */
  ...
  public final IInformationProvider getInformationProvider() {
    if (informationProvider == null) {
      informationProvider =
          new ModuleInformationProvider(getModuleIdProvider(), generateInformationCardCustomizer());
    }
  ...
  /**
   * If you want to provide "pull" based "current" module information to the user, then implement
   * this class by instantiating your InformationProvider. If you don't plan such a feature, then
   * just return null.
   * 
   * @return
   */
  ...
  /**
   * Implement this if your module should provide a custom REST service, e.g. for 3party Apps to
   * communicate directly with your module. Otherwise just return null.
   * 
   * @return
   */
